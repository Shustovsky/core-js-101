...
const removeLeadingAndTrailingWhitespaces = (value) => value.trimStart()
  .trimEnd();
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    if (i === 0) {
      const addSymbols = '─'.repeat(+width - 2);
      result += `┌${addSymbols}┐\n`;
    } else if (i === height - 1) {
      const addSymbols = '─'.repeat(+width - 2);
      result += `└${addSymbols}┘\n`;
...
const encodeToRot13 = (str) => {
  const alphabetPart1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const alphabetPart2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    if (alphabetPart1.includes(str[i])) {
      const index = alphabetPart1.indexOf(str[i]);
      result += alphabetPart2.charAt(index);
    } else {
      result += str[i];
    }
  }
